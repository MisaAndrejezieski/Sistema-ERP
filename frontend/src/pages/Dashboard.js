...
export default function Dashboard() {
  ...
  const [stats, setStats] = useState({ clientes: 0, vendas: 0, produtos: 0, usuarios: 0, valorTotal: 0 });
      ...
      <nav style={{
        ...
        <div style={{ display: 'flex', gap: '24px', flexWrap: 'wrap' }}>
          {[
            ...
            ...(usuario?.cargo === 'gerente' ? [{ href: '/usuarios', label: '👤 Equipe' }, { href: '/logs', label: '📋 Logs' }] : []),
          ].map(item => (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '20px', marginBottom: '28px' }}>
          {cards.map(card => (
            <a key={card.titulo} href={card.link} style={{ textDecoration: 'none' }}>
              <div className="card" style={{ background: card.cor, color: 'white', cursor: 'pointer' }}>
          ...
          <div className="card" style={{ gridColumn: '1 / -1' }}>
            ...
            <ResponsiveContainer width="100%" height={280}>
              <LineChart data={vendasMensais}>
                ...
                <Tooltip />
                <Legend />
                <Line type="monotone" dataKey="valor" stroke="#10b981" strokeWidth={3} dot={{ r: 6 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
